...
The selected deployment is to deploy **all three application containers — `webapp`, `auth-server`, `inference` — as Cloud Run services**, keeping only MongoDB on Atlas. The existing `webapp/Dockerfile` and `nginx.conf` provide the starting point; nginx serves the built SPA and reverse-proxies `/api/auth/` and `/api/` to the two backend services. Compose supplies Docker service URLs locally, while Cloud Run supplies each service URI through environment variables.
...
- **Cloud Run still needs upstream wiring.** The deployment workflow reads each backend's `status.url`, and Terraform passes each service URI to the webapp. The startup template renders those values before nginx starts, with HTTPS SNI and the upstream Host header enabled; no variable-based nginx `resolver` is needed.
