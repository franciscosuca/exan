"""Tests for structured grammar evaluation results."""
...
from app.services.grammar_evaluation import GrammarEvaluationError, GrammarEvaluationService
...
def _service(provider: FakeProvider) -> GrammarEvaluationService:
    return GrammarEvaluationService(lambda _provider, _model: provider)
...
async def test_maps_grammar_issues_and_summary():
    ...
    with patch("app.services.grammar_evaluation.write_run_log"):
...
@pytest.mark.asyncio
async def test_malformed_grammar_output_raises_grammar_evaluation_error():
    provider = FakeProvider(
        [
            {
                "grammar": {
