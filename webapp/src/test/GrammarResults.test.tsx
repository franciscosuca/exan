...
function renderResults(response: GrammarEvaluationResponse) {
  return render(
    <LanguageProvider>
      <GrammarResults response={response} />
    </LanguageProvider>
  );
}

function createResponse(
  result: Omit<GrammarEvaluationResponse['results'][number], 'id'> & { id?: string }
): GrammarEvaluationResponse {
  return {
    id: 'grammar-evaluation-1',
